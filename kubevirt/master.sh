...
echo "[TASK 12] Dynamically provisioning persistent local storage with Kubernetes - v0.12.0"
curl -L https://raw.githubusercontent.com/kubevirt/hostpath-provisioner/main/deploy/kubevirt-hostpath-provisioner.yaml -o /root/hostpath-provisioner.yaml >/dev/null 2>&1
sed -i "s|v1beta1|v1|" /root/hostpath-provisioner.yaml 
kubectl apply -f /root/hostpath-provisioner.yaml >/dev/null 2>&1
kubectl annotate storageclass kubevirt-hostpath-provisioner storageclass.kubernetes.io/is-default-class=true >/dev/null 2>&1
